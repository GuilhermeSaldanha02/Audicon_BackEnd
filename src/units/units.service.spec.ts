...
describe('UnitsService', () => {
  ...
  describe('create', () => {
    ...
    it('deve propagar NotFoundException quando condomínio não existir', async () => {
      ...
      condominiumsService.findOne.mockRejectedValue(
        new NotFoundException('Condo not found'),
      );
      await expect(service.create(condoId, dto)).rejects.toBeInstanceOf(
        NotFoundException,
      );
  ...
  describe('findAll', () => {
    ...
    it('deve lançar NotFoundException quando condomínio não existir', async () => {
      ...
      await expect(service.findAll(condoId)).rejects.toBeInstanceOf(
        NotFoundException,
      );
  ...
  describe('update', () => {
    ...
    it('deve lançar NotFoundException quando unidade não existir', async () => {
      repository.findOneBy.mockResolvedValue(null);
      await expect(
        service.update(999, { residentName: 'X' } as any),
      ).rejects.toBeInstanceOf(NotFoundException);
      expect(repository.update).not.toHaveBeenCalled();
    });
  });
  describe('remove', () => {
    it('deve soft-deletar com sucesso (retorna undefined)', async () => {
      const id = 1;
